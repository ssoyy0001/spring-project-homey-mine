   ...
   <main id="main">
      ...
      <div class="breadcrumbs d-flex align-items-center"
         ...
         <div
            ...
            data-aos="fade">
            <h2>
               <a href="/consult/list">견적상담 전체 조회</a>
            </h2>
            <!-- 클릭시 초기 리스트로 -->
         </div>
      </div>
            ...
            <div class="col-lg-9 mx-auto">
               ...
               <form action="/consult/list" id="searchFrm">
                  <select name="type" style="height: 30px">
                     <!-- 검색어 입력칸과 높이 맞추기 -->
                     <option value="C"
                        <c:if test="${pageDTO.cri.type == 'C'}">selected</c:if>>견적상담번호</option>
                     <!--  검색 타입 남겨놓기-->
                     <option value="B"
                        <c:if test="${pageDTO.cri.type == 'B'}">selected</c:if>>건물유형</option>
                  </select> <input type="text" name="keyword" value="${pageDTO.cri.keyword }"
                     placeholder="상담번호는 숫자만 입력">
                  <!-- 검색 키워드 남겨놓기 -->
               ...
               <%@ include file="../includes/sideMenuAdm.jsp"%></div>
         </div>
      </section>

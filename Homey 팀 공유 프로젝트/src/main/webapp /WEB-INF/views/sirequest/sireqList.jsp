					...
					<div class="row">
						...
						<form action="/sirequest/sireqList" id="searchFrm">
							<select name="type">
								...
								<option value="Q" ${type == 'Q' ? 'selected' : '' }>견적서 번호</option>
...
<div class="row">
	...
	<!-- Model 데이터 출력 -->
			<c:forEach items="${list}" var="sireq">
				<tr>
					<td><a class="move" href="${sireq.sireqNo}">
						<c:out value="${sireq.sireqNo}" /></a></td>
					...
					<td><fmt:formatDate value="${sireq.sireqUpdateDate}"
							pattern="yyyy-MM-dd" /></td>
				</tr>
			</c:forEach>
						<!-- END Model 데이터 출력 -->
				...
				<div class="pull-right">
					<ul class="pagination">
						...
						<c:if test="${pageDTO.prev }">
							...
							<a href="${pageDTO.start - 1}">&laquo; Previous</a>
							</li>
						</c:if>
						...
						<c:if test="${pageDTO.next }">
							<li class="paginate_button next"><a href="${pageDTO.end + 1}"> Next &raquo;</a>
							</li>
						</c:if>
					</ul>
				</div>
				...
				<form action="/sirequest/sireqList" id="actionFrm" method="get">
	...
	$(document).ready(function(){
		...
		function checkModal(result) {
			...
			if (parseInt(result) > 0) { //게시물이 등록된 경우
				$('.modal-body').html(parseInt(result) + '번 시공의뢰가 등록되었습니다.');
			}
	...
	$(function() {
		...
		$('.move').on(
				...
				function(e) {
					...
					actionFrm.append("<input type='hidden' name='sireqNo' value='"
							+ $(this).attr('href') + "'>");

					actionFrm.attr('action', '/sirequest/sireqView');

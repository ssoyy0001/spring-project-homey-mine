...
<section id="project-details" class="project-details">
    ...
    <div class="col-lg-8 mx-auto">
	    <div class="portfolio-description">
	        <h2>📝원데이클래스 참여후기 작성</h2>
	        <span id="notice">* 항목은 필수 입력 항목입니다.</span>
	        <hr>
<!-----------------------start form----------------------->
	        <form action="/odReview/register" method="post" id="regiForm" role="form" enctype="multipart/form-data">
	            ...
	            <div class="form-group mt-3">
	                <label class="regiItemNm"><strong>* 참여클래스</strong></label>
	                <input type="text" class="staticItem" value="${odName }" readonly>
	            </div>

	            <div class="form-group mt-3">
	                <label class="regiItemNm"><strong>* 제   목</strong></label>
	                <input type="text" name="orTitle" class="form-control" required>
	            </div>
	            
	            <div class="form-group mt-3">
		            <label class="regiItemNm"><strong>* 내   용</strong></label>
		            <textarea class="form-control" name="orContent" id="" rows="10"></textarea>
	        	</div>
...
//코드 작성 순서 :   1) showUploadedFile함수에서 li태그에 data- 내용 담고  
//2) 그 내용 가지고 form태그에 hidden태그 만듦  , frm.submit() 작성
//3) 게시글 등록 시 attach테이블에도 insert되는지 확인
//=>F12 눌렀을 때 form태그에 hidden태그가 생겨야 함
...
//원데이클래스 후기작성 폼에서 [등록] 버튼 클릭 시
$('.regBtn').click(function(event) {
	event.preventDefault(); 						//form 제출 막기

	
	// 필수 입력 필드의 값들을 가져와서 검사
	var orTitle = $('input[name="orTitle"]').val();
	var orContent = $('input[name="orContent"]').val();

	if ( orTitle === "" || orContent === "" ) {		//입력 필수항목 중 빈 값이 있을 경우
		...
		$('.uploadResult ul li').each(function(i, obj){
	        hiddenTag += '<input type="hidden" name="orAttachList[' + i + '].uuid" value="' + $(obj).data('uuid') + '">' +
            '<input type="hidden" name="orAttachList[' + i + '].fileName" value="' + $(obj).data('filenm') + '">' +
            '<input type="hidden" name="orAttachList[' + i + '].upFolder" value="' + $(obj).data('folder') + '">' +
            '<input type="hidden" name="orAttachList[' + i + '].image" value="' + $(obj).data('image') + '">';
		});
			 
		$('#regiForm').append(hiddenTag);		//form안에 hidden 태그 붙이기
...
function showUploadedFile(result){
	...
	$(result).each(function(i, obj){		//i는 result(attachList)배열의 크기, obj는 result 속 개별파일
		...
		}else{	//일반 파일(사진파일이 아닌 경우)은 attach.png 이미지와 파일명 표시하고, 클릭 시 파일이 다운로드되도록 구현
			var filePath = encodeURIComponent(obj.upFolder + "/" + obj.uuid + "_" + obj.fileName);		//다운로드 할 수 있게 하는 경로
			liTag += '<a href="/download?fileName=' + filePath + '">'
	           + '<img src="/resources/img/attach.png"></a><br>'
	           + obj.fileName
	           +'<button type="button" class="btn btn-warning btn-circle" '
	           +'data-file="' + filePath + '" ' + ' data-type="file"> '
	           +' <i class="fa fa-times"></i></button></li>';
		}
	});//END each()
